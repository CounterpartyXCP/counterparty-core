...
def parse_hex (unsigned_tx_hex):
    ...
    source, destination, btc_amount, fee, data = util.aio_wait_for(blocks.get_tx_info(tx,
        block_index))
...
def test_burn ():
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(burn.compose(db,
...
def test_send ():
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(send.compose(db,
...
def test_order_buy_xcp ():
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(order.compose(db,
...
def test_btcpay ():
    ...
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(btcpay.compose(db,
...
def test_issuance_divisible ():
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(issuance.compose(db,
...
def test_dividend_divisible ():
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(dividend.compose(db,
...
def test_broadcast_initial ():
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(broadcast.compose(db,
...
def test_bet_bullcfd_to_be_liquidated ():
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(bet.compose(db, source_default,
...
def test_cancel ():
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(cancel.compose(db,
        source_default,
...
def test_callback ():
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(callback.compose(db,
...
def test_counter_rps ():
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(rps.compose(db,
...
def test_rpsresolve ():
    ...
    unsigned_tx_hex = util.aio_wait_for(bitcoin.transaction(rpsresolve.compose(db,
...
#     })
    ...
    for payload in payloads:
        for attempt in range(100):  # Try until server is ready.
            try:
                response = util.aio_wait_for(aiohttp.request('POST', url,
                        data=json.dumps(payload), headers=headers))
                response = util.aio_wait_for(response.json())
                print(response)
                # print('\npayload', payload)
