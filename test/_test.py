...
def parse_hex (unsigned_tx_hex, getrawtransaction_db):
    ...
    if pytest.config.option.saverawtransactions:
        util_test.save_getrawtransaction_data(getrawtransaction_db, tx_hash, unsigned_tx_hex)
...
def test_order_buy_xcp (getrawtransaction_db):
...
def test_order_sell_xcp (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, order.compose(db,
...
def test_btcpay (getrawtransaction_db):
...
def test_issuance_divisible (getrawtransaction_db):
...
def test_issuance_indivisible_callable (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, issuance.compose(db,
...
def test_send_divisible (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, send.compose(db,
...
def test_send_indivisible (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, send.compose(db,
...
def test_dividend_divisible (getrawtransaction_db):
...
def test_dividend_indivisible (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, dividend.compose(db,
...
def test_broadcast_initial (getrawtransaction_db):
...
def test_bet_bullcfd_to_be_liquidated (getrawtransaction_db):
...
def test_bet_bearcfd_to_be_liquidated (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, bet.compose(db, source_default,
...
def test_bet_bullcfd_to_be_settled (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, bet.compose(db, source_default,
...
def test_bet_bearcfd_to_be_settled (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, bet.compose(db, source_default,
...
def test_bet_equal (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, bet.compose(db, source_default,
...
def test_bet_notequal (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, bet.compose(db, source_default,
...
def test_broadcast_liquidate (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, broadcast.compose(db,
...
def test_broadcast_settle (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, broadcast.compose(db,
...
def test_broadcast_equal (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, broadcast.compose(db,
...
def test_order_to_be_cancelled (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, order.compose(db,
...
def test_cancel (getrawtransaction_db):
...
def test_overburn (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, burn.compose(db,
...
def test_send_callable (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, send.compose(db,
...
def test_callback (getrawtransaction_db):
...
def test_rps (getrawtransaction_db):
...
def test_counter_rps (getrawtransaction_db):
...
def test_rpsresolve (getrawtransaction_db):
...
def test_counter_rpsresolve (getrawtransaction_db):
    rps_match_id = '58f7b0780592032e4d8602a3e8690fb2c701b2e1dd546e703445aabd6469734d77adfc95029e73b173f60e556f915b0cd8850848111358b1c370fb7c154e61fd'
    unsigned_tx_hex = bitcoin.transaction(db, rpsresolve.compose(db,
...
def test_rps_expiration (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, rps.compose(db, source_default,
        5, 11021663, move_random_hash_default, 10), encoding='multisig')
    tx_rps = parse_hex(unsigned_tx_hex, getrawtransaction_db)
...
def test_pending_rps_match_expiration (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, rps.compose(db, source_default,
        5, 11021664, move_random_hash_default, 10), encoding='multisig')
    rps1 = parse_hex(unsigned_tx_hex, getrawtransaction_db)
    ...
    unsigned_tx_hex = bitcoin.transaction(db, rps.compose(db,
        ...
        encoding='multisig')
    rps2 = parse_hex(unsigned_tx_hex, getrawtransaction_db)
...
def test_pending_and_resolved_rps_match_expiration (getrawtransaction_db):
    unsigned_tx_hex = bitcoin.transaction(db, rps.compose(db, source_default,
    ...
    unsigned_tx_hex = bitcoin.transaction(db, rpsresolve.compose(db,
        source_default, 3, rps_random_default, rps_match_id),
        encoding='multisig')
    rps_match = parse_hex(unsigned_tx_hex, getrawtransaction_db)
