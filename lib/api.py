...
class APIServer(threading.Thread):
    ...
    def run (self):
        ...
        leverage=5040, encoding='multisig', pubkey=None, unconfirmed_change=False):
            try:
            ...
            tx_info = bet.compose(db, source, feed_address,
                              ...
                              leverage, expiration)
            return bitcoin.transaction(tx_info, encoding=encoding, public_key_hex=pubkey, unconfirmed_change=unconfirmed_change)

        @dispatcher.add_method
        def create_broadcast(source, fee_fraction, text, timestamp, value=-1, encoding='multisig', pubkey=None, unconfirmed_change=False):
            tx_info = broadcast.compose(db, source, timestamp,
                                    value, fee_fraction, text)
            return bitcoin.transaction(tx_info, encoding=encoding, public_key_hex=pubkey, unconfirmed_change=unconfirmed_change)

        @dispatcher.add_method
        def create_btcpay(order_match_id, encoding='multisig', pubkey=None, unconfirmed_change=False):
            tx_info = btcpay.compose(db, order_match_id)
            return bitcoin.transaction(tx_info, encoding=encoding, public_key_hex=pubkey, unconfirmed_change=unconfirmed_change)

        @dispatcher.add_method
        def create_burn(source, quantity, encoding='multisig', pubkey=None, unconfirmed_change=False):
            tx_info = burn.compose(db, source, quantity)
            return bitcoin.transaction(tx_info, encoding=encoding, public_key_hex=pubkey, unconfirmed_change=unconfirmed_change)

        @dispatcher.add_method
        def create_cancel(offer_hash, encoding='multisig', pubkey=None, unconfirmed_change=False):
            tx_info = cancel.compose(db, offer_hash)
            return bitcoin.transaction(tx_info, encoding=encoding, public_key_hex=pubkey, unconfirmed_change=unconfirmed_change)

        @dispatcher.add_method
        def create_callback(source, fraction, asset, encoding='multisig', pubkey=None, unconfirmed_change=False):
            tx_info = callback.compose(db, source, fraction, asset)
            return bitcoin.transaction(tx_info, encoding=encoding, public_key_hex=pubkey, unconfirmed_change=unconfirmed_change)

        @dispatcher.add_method
        def create_dividend(source, quantity_per_unit, asset, dividend_asset, encoding='multisig', pubkey=None, unconfirmed_change=False):
        ...
        call_price=None, transfer_destination=None, lock=False, encoding='multisig', pubkey=None, unconfirmed_change=False):
            try:
        ...
        fee_provided=None, encoding='multisig', pubkey=None, unconfirmed_change=False):
        ...
        @dispatcher.add_method
        def create_send(source, destination, asset, quantity, encoding='multisig', pubkey=None, unconfirmed_change=False):
