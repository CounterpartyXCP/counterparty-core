...
from lib import config, exceptions, util, backend, logger
...
def cancel_order (db, order, status, block_index):
    ...
    logger.message(db, block_index, 'update', 'orders', bindings)
...
def cancel_order_match (db, order_match, status, block_index):
    ...
    cursor.execute(sql, bindings)
    logger.message(db, block_index, 'update', 'order_matches', bindings)
