...
def compose (db, source, give_asset, give_quantity, get_asset, get_quantity, expiration, fee_required):
    ...
    give_id = util.asset_id(give_asset, util.last_block(db)['block_index'])
    get_id = util.asset_id(get_asset, util.last_block(db)['block_index'])
...
def parse (db, tx, message):
    ...
    try:
        ...
        give_asset = util.asset_name(give_id, tx['block_index'])
        get_asset = util.asset_name(get_id, tx['block_index'])
        status = 'open'
