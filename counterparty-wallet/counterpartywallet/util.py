...
def api(method, params=None):
    return rpc(
        ...
        method,
        params=params,
        ssl_verify=config.COUNTERPARTY_RPC_SSL_VERIFY,
    )


def wallet_api(method, params=None):
    return rpc(
        config.WALLET_URL, method, params=params, ssl_verify=config.WALLET_SSL_VERIFY
    )
...
def is_divisible(asset):
    if asset in (
        ...
        if not issuances:
            raise AssetError(f"No such asset: {asset}")
