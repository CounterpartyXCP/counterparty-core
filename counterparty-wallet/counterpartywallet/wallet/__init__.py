...
from counterpartycore.lib import config, exceptions, script, util  # noqa: E402, F401
...
def asset(asset_name):
    ...
    if asset_name in ["XCP", "BTC"]:
        asset_info.update(
            {"owner": None, "divisible": True, "locked": False, "description": "", "issuer": None}
        )
    else:
        issuances = api(
            "get_issuances",
    ...
    for bunch in get_btc_balances():
        ...
        if asset_name == "BTC":
            ...
            balances = api(
                "get_balances",
                {"filters": [("address", "==", address), ("asset", "==", asset_name)]},
            )
            if balances:
                balance = balances[0]
...
def pending():
    ...
    filters = [("tx0_address", "IN", addresses), ("tx1_address", "IN", addresses)]
    awaiting_btcs = api(
        "get_order_matches", {"filters": filters, "filterop": "OR", "status": "pending"}
    )
