...
CONFIG_ARGS = [
    ...
    [
        ...
        {
            "default": "localhost",
            "help": "the hostname or IP of the Counterparty JSON-RPC server",
        },
    ],
    [
    ...
    [
        ("--wallet-name",),
        {"default": "bitcoincore", "help": "the wallet name to connect to"},
    ],
    [
        ("--wallet-connect",),
    ...
    [
        ("--disable-utxo-locks",),
        {
            "action": "store_true",
            "default": False,
            "help": "disable locking of UTXOs being spend",
        },
    ],
    [
        ("--dust-return-pubkey",),
        {"help": "pubkey for dust outputs (required for P2SH)"},
    ],
    [
        ("--requests-timeout",),
        {
...
def main():
    ...
    parser_send.add_argument(
        "--destination", required=True, help="the destination address"
    )
    parser_send.add_argument(
        "--quantity", required=True, help="the quantity of ASSET to send"
    )
    parser_send.add_argument(
        "--asset",
        required=True,
    ...
    parser_send.add_argument(
        "--fee", help=f"the exact {config.BTC} fee to be paid to miners"
    )

    parser_sweep = subparsers.add_parser(
        "sweep", help="create and broadcast a *sweep* message"
    )
    ...
    parser_dispenser = subparsers.add_parser(
        "dispenser", help="create and broadcast a *dispenser*"
    )
    parser_dispenser.add_argument("--source", required=True, help="the source address")
    parser_dispenser.add_argument(
        "--asset",
        required=True,
        help="the ASSET of which you would like to dispense GIVE_QUANTITY",
    )
    parser_dispenser.add_argument(
        "--mainchainrate",
        required=True,
    ...
    parser_issuance.add_argument(
        "--quantity", default=0, help="the quantity of ASSET to be issued"
    )
    parser_issuance.add_argument(
        "--asset",
        required=True,
    ...
    parser_bet.add_argument(
        "--wager", required=True, help="the quantity of XCP to wager"
    )
    parser_bet.add_argument(
        "--counterwager",
        required=True,
    ...
    parser_dividend.add_argument(
        "--asset", required=True, help="the asset to which pay dividends"
    )
    parser_dividend.add_argument(
    ...
    parser_publish.add_argument(
        "--gasprice", required=True, type=int, help="the price of gas"
    )
    parser_publish.add_argument(
        "--startgas",
        required=True,
    ...
    parser_pending = subparsers.add_parser(
        "pending",
        help=f"list pending order matches awaiting {config.BTC}payment from you",
    )
    ...
    parser_get_tx_info = subparsers.add_parser(
        "get_tx_info", help="display info of a raw TX"
    )
