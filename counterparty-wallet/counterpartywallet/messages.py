...
def get_pubkey_monosig(pubkeyhash, pubkey_resolver=input_pubkey):
    if wallet.is_valid(pubkeyhash):
        ...
        try:
            pubkey = util.api(
                "search_pubkey", {"pubkeyhash": pubkeyhash, "provided_pubkeys": None}
            )
        ...
        if is_fully_valid_pubkey:
            ...
            logging.debug(
                "Answer was not a fully valid public key. Assuming answer was a private key."
            )
            private_key = answer
            try:
        ...
        if pubkeyhash != script.pubkey_to_pubkeyhash(
            ...
            raise InputError(
                "provided public or private key does not match the source address"
            )
...
def prepare_args(args, action):
    ...
    if action == "order":
        ...
        if args.give_asset == config.BTC:
            ...
            print(
                f"Fee provided: {util.value_out(args.fee_provided, config.BTC)} {config.BTC}"
            )
        elif args.get_asset == config.BTC:
            args.fee_provided = 0
            fee_fraction_required = util.value_in(
                args.fee_fraction_required, "fraction"
            )
            args.fee_required = round(
                D(fee_fraction_required) * D(get_quantity) * D(config.UNIT)
            )
            print(
                f"Fee required: {util.value_out(args.fee_required, config.BTC)} {config.BTC}"
            )
        else:
            args.fee_required = 0
            args.fee_provided = 0
...
def check_transaction(method, params, tx_hex):
    ...
    fee_per_kb = (
        params["fee_per_kb"] if "fee_per_kb" in params else config.DEFAULT_FEE_PER_KB
    )
...
def compose_transaction(args, message_name, param_names):
    ...
    for address_name in ["source", "destination"]:
        if address_name in params:
            ...
            if not script.is_p2sh(address) and (
