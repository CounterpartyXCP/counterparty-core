...
def test_search_raw_transactions_output():
    ...
    # general
    assert (
        tx["txid"] == "02f95716d3c93a1e81b926d9d8d5f05b6f382c115d9ecf0dd0bc9514b0e08649"
    )
    ...
    assert tx["vout"][1]["scriptPubKey"]["addresses"] == [
        "mqPCfvqTfYctXMUfmniXeG2nyaN8w6tPmj"
    ]
    assert (
...
def test_search_raw_transactions_unconfirmed(server_db):
    ...
    send1hex = util.api(
        "create_send",
        {"source": ADDR[0], "destination": ADDR[1], "asset": "XCP", "quantity": v},
    )
