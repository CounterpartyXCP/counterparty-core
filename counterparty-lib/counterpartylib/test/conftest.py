...
def init_mock_functions(request, monkeypatch, mock_utxos, rawtransactions_db):
    ...
    monkeypatch.setattr(
        "counterpartylib.lib.backend.getrawtransaction_batch", mocked_getrawtransaction_batch
    )
    monkeypatch.setattr(
        "counterpartylib.lib.backend.search_raw_transactions", mocked_search_raw_transactions
    )
