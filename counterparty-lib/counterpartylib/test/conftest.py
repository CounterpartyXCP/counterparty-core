...
def pytest_generate_tests(metafunc):
    ...
    if metafunc.function.__name__ == "test_vector":
        ...
        metafunc.parametrize(
            "scenario_name, base_scenario_name, transactions, pytest_config", args
        )
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--function", action="append", default=[], help="List of functions to test"
    )
    parser.addoption(
        "--scenario", action="append", default=[], help="List of scenarios to test"
    )
    parser.addoption(
        "--gentxhex",
        action="store_true",
    ...
    parser.addoption(
        ...
        action="store_true",
        default=False,
        help="Include testnet test book",
    )
...
class MockUTXOSet(object):
    ...
    def get_unspent_txouts(
        ...
        if unconfirmed == False:
            unspent_txouts = filter(
                lambda txout: txout["confirmations"] > 0, unspent_txouts
            )
        ...
        if unspent_tx_hash:
            unspent_txouts = filter(
                lambda txout: txout["txid"] == unspent_tx_hash, unspent_txouts
            )
    ...
    def increment_confirmations(self):
        ...
        cursor.execute(
            """UPDATE raw_transactions SET confirmations = confirmations + 1"""
        )
        cursor.close()
...
def mock_utxos(rawtransactions_db):
    with open(
        util_test.CURR_DIR + "/fixtures/unspent_outputs.json", "r"
    ) as listunspent_test_file:
        util_test.MOCK_UTXO_SET = MockUTXOSet(
            json.load(listunspent_test_file), rawtransactions_db
        )
...
def init_mock_functions(request, monkeypatch, mock_utxos, rawtransactions_db):
    ...
    def multisig_pubkeyhashes_to_pubkeys(address, provided_pubkeys=None):
        ...
        address = "_".join(
            [str(signatures_required)] + sorted(pubkeys) + [str(len(pubkeys))]
        )
        return address
    ...
    def mocked_getrawtransaction_batch(txhash_list, verbose=False, skip_missing=False):
        return util_test.getrawtransaction_batch(
            rawtransactions_db, txhash_list, verbose=verbose
        )

    def mocked_search_raw_transactions(address, unconfirmed=False):
        return util_test.search_raw_transactions(
            rawtransactions_db, address, unconfirmed
        )
    ...
    monkeypatch.setattr(
        "counterpartylib.lib.backend.get_unspent_txouts", get_unspent_txouts
    )
    ...
    monkeypatch.setattr(
        "counterpartylib.lib.api.init_api_access_log", init_api_access_log
    )
    if hasattr(config, "PREFIX"):
        monkeypatch.setattr("counterpartylib.lib.config.PREFIX", b"TESTXXXX")
    monkeypatch.setattr(
        "counterpartylib.lib.backend.getrawtransaction", mocked_getrawtransaction
    )
    monkeypatch.setattr(
        "counterpartylib.lib.backend.getrawtransaction_batch",
        mocked_getrawtransaction_batch,
    )
    monkeypatch.setattr(
        "counterpartylib.lib.backend.search_raw_transactions",
        mocked_search_raw_transactions,
    )
    monkeypatch.setattr(
        "counterpartylib.lib.backend.pubkeyhash_to_pubkey", pubkeyhash_to_pubkey
    )
    monkeypatch.setattr(
        "counterpartylib.lib.backend.multisig_pubkeyhashes_to_pubkeys",
