...
def test_p2sh_encoding(server_db):
    ...
    with util_test.ConfigContext(
        ...
        sumvin = sum(
            [
                int(
                    utxos[(bitcoinlib.core.b2lx(vin.prevout.hash), vin.prevout.n)][
                        "amount"
                    ]
                    * 1e8
                )
        ...
        assert not (
            binascii.hexlify(othertxid).decode("ascii") == pretxid
        ...
        tx_script = datatxhex[
            tx_script_start + 2 : tx_script_start + 2 + tx_script_length
        ]
...
def test_p2sh_encoding_long_data(server_db):
    ...
    with util_test.ConfigContext(
        ...
        assert (
            pretx.vout[2].nValue
            == sumvin - (2 * config.DEFAULT_REGULAR_DUST_SIZE) - pretx_fee
        )
...
def test_p2sh_script_decoding():
    ...
    print(
        "redeemScript", binascii.hexlify(redeemScript) if redeemScript else redeemScript
    )
