...
def test_scenario(scenario_name, base_scenario_name, transactions, pytest_config):
    ...
    if pytest_config.getoption("savescenarios"):
        util_test.save_scenario_output(
            scenario_name, new_dump, new_log, new_raw_transactions
        )

    old_dump, old_log, old_raw_transactions = util_test.load_scenario_ouput(
        scenario_name
    )
    ...
    if base_scenario_name and base_scenario_name != scenario_name:
        base_dump, base_log, base_raw_transactions = util_test.load_scenario_ouput(
            base_scenario_name
        )
