...
from counterpartylib.lib.backend.indexd import (
    extract_addresses,
...
def init_database(sqlfile, dbfile, options=None):
    ...
    db = database.get_connection(
        read_only=False
    )  # reinit the DB to deal with the restoring
...
def create_next_block(db, block_index=None, parse_block=True):
    ...
    if last_block_index >= block_index:
        raise Exception(
            "create_next_block called with height lower than current height"
        )
...
def insert_raw_transaction(raw_transaction, db):
    ...
    try:
        ...
        tx = list(
            cursor.execute(
                """SELECT * FROM transactions WHERE tx_index = ?""", (tx_index,)
            )
        )[0]
...
def dummy_tx_hash(raw_transaction):
    ...
    if tx_id in UNIQUE_DUMMY_TX_HASH:
        ...
        tx_id = hashlib.sha256(
            f"{tx_id}{UNIQUE_DUMMY_TX_HASH[tx_id]}".encode("utf-8")
        ).hexdigest()
    else:
...
def prefill_rawtransactions_db(db):
    ...
    with open(
        CURR_DIR + "/fixtures/unspent_outputs.json", "r"
...
def getrawtransaction(db, txid, verbose=False, block_index=None):
    ...
    tx_hex, confirmations = list(
        cursor.execute(
            """SELECT tx_hex, confirmations FROM raw_transactions WHERE tx_hash = ?""",
            (txid,),
        )
    )[0]
    cursor.close()
...
def mock_bitcoind_verbose_tx_output(tx, txid, confirmations):
    ...
    for vin in ctx.vin:
        asm = list(
            map(
                lambda op: (
                    binascii.hexlify(op).decode("ascii")
    ...
    for idx, vout in enumerate(ctx.vout):
        if list(vout.scriptPubKey)[-1] == bitcoinlib.core.script.OP_CHECKMULTISIG:
            ...
            try:
                addresses = [
                    str(
                        bitcoinlib.wallet.CBitcoinAddress.from_scriptPubKey(
                            vout.scriptPubKey
                        )
                    )
                ]
...
def save_scenario_output(scenario_name, dump, log, raw_transactions):
    ...
    with open(CURR_DIR + "/fixtures/scenarios/" + scenario_name + ".new.log", "w") as f:
        f.writelines(log)
    with open(
        CURR_DIR + "/fixtures/scenarios/" + scenario_name + ".new.json", "w"
    ) as f:
...
def clean_scenario_dump(scenario_name, dump):
    ...
    dump = dump.replace(
        standard_scenarios_params[scenario_name]["address1"], "address1"
    )
    dump = dump.replace(
        standard_scenarios_params[scenario_name]["address2"], "address2"
    )
    ...
    with MockProtocolChangesContext(**(mock_protocol_changes or {})):
        ...
        if error is not None:
            ...
            if pytest_config.getoption("gentxhex") and method == "compose":
                ...
                unsigned_tx_hex = transaction.construct(
                    server_db, test_outputs, **tx_params
                )
                print(tx_name)
...
def get_block_ledger(db, block_index):
    ...
    cursor = db.cursor()
    debits = list(
        cursor.execute("""SELECT * FROM debits WHERE block_index = ?""", (block_index,))
    )
    credits = list(
        cursor.execute(
            """SELECT * FROM credits WHERE block_index = ?""", (block_index,)
        )
    )
...
def get_block_txlist(db, block_index):
    ...
    txlist = list(
        cursor.execute(
            """SELECT * FROM transactions WHERE block_index = ?""", (block_index,)
        )
    )
...
def reparse(testnet=True, checkpoint_count=5):
    ...
    prod_db_path = os.path.join(
        data_dir, f"{config.APP_NAME}{'.testnet' if testnet else ''}.db"
    )
