...
def get_lock():
    ...
    if os.name == "nt" or platform.system() == "Darwin":  # Windows or OS X
        ...
        error = (
            f"Another copy of server is currently writing to database {config.DATABASE}"
        )
...
def initialise_config(
    ...
    cleaned_backend_url = config.BACKEND_URL.replace(
        f":{config.BACKEND_PASSWORD}@", ":*****@"
    )
    ...
    config.INDEXD_URL = (
        "http://" + config.INDEXD_CONNECT + ":" + str(config.INDEXD_PORT)
    )
...
def configure_rpc(rpc_password=None):
    ...
    cleaned_rpc_url = config.RPC.replace(
        f":{urlencode(config.RPC_PASSWORD)}@", ":*****@"
    )
...
the `bootstrap` command should not be used for mission-critical, commercial or public-facing nodes.
    ...
    urllib.request.urlretrieve(
        bootstrap_url, tarball_path, bootstrap_progress
    )  # nosec B310
    spinner.stop()
    print(f"{OK_GREEN} {step}")
