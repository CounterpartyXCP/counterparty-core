...
def get_messages(db, block_index=None, block_index_in=None, message_index_in=None):
    ...
    if block_index_in is not None:
        where.append(
            f"block_index IN ({','.join(['?' for e in range(0, len(block_index_in))])})"
        )
...
def debit(db, address, asset, quantity, tx_index, action=None, event=None):
    ...
    if asset == config.BTC:
        raise exceptions.BalanceError(
            f"Cannot debit bitcoins from a {config.XCP_NAME} address!"
        )
...
def get_balance(db, address, asset, raise_error_if_no_balance=False, return_list=False):
    ...
    if not balances and raise_error_if_no_balance:
        raise exceptions.BalanceError(
            f"No balance for this address and asset: {address}, {asset}."
        )
    if not balances:
        return 0
...
def value_out(db, quantity, asset, divisible=None):
    if (
        asset not in ["leverage", "value", "fraction", "price", "odds"]
        and divisible == None
    ):
...
def price(numerator, denominator):
    ...
    if (
        CURRENT_BLOCK_INDEX >= 294500 or config.TESTNET or config.REGTEST
    ):  # Protocol change.
...
def get_transactions(db, tx_hash=None):
    ...
    # no sql injection here
    query = (
        f"""SELECT * FROM transactions WHERE ({" AND ".join(where)})"""  # nosec B608
    )
    cursor.execute(query, tuple(bindings))
    return cursor.fetchall()
...
def insert_update(db, table_name, id_name, id_value, update_data, event, event_info={}):
    ...
    add_to_journal(
        db, CURRENT_BLOCK_INDEX, "update", table_name, event, update_data | event_paylod
    )
...
def get_pending_dispensers(db, delay, block_index):
    ...
    return [
        dispenser
        for dispenser in cursor.fetchall()
        if dispenser["status"] == status_closing
    ]
...
def get_dispensers(
    ...
    if source_in is not None:
        first_where.append(
            f"source IN ({','.join(['?' for e in range(0, len(source_in))])})"
        )
        bindings += source_in
    if asset is not None:
    ...
    if status_in is not None:
        second_where.append(
            f"status IN ({','.join(['?' for e in range(0, len(status_in))])})"
        )
        bindings += status_in
    # build query
    ...
    order_clause = (
        f"ORDER BY {order_by}" if order_by is not None else "ORDER BY tx_index"
    )
    group_clause = (
        f"GROUP BY {group_by}" if group_by is not None else "GROUP BY asset, source"
    )
    # no sql injection here
    query = f"""
        SELECT *, rowid FROM (
...
def get_bet_matches_to_expire(db, block_time):
    ...
    bindings = (
        ...
        block_time
        - 2
        * config.TWO_WEEKS,  # optimize query: assuming before that we have already expired
        "pending",
    )
    cursor.execute(query, bindings)
    return cursor.fetchall()
...
def get_matching_rps(db, possible_moves, wager, source, already_matched_tx_hashes):
    ...
    if already_matched_tx_hashes:
        place_holders = ",".join(
            ["?" for e in range(0, len(already_matched_tx_hashes))]
        )
...
def _get_holders(
    ...
    for holder in save_records.values():
        if holder[hold_fields_1["address_quantity"]] > 0 or (
            exclude_empty_holders == False
            and holder[hold_fields_1["address_quantity"]] == 0
        ):
            holders.append(
                {
        ...
        if hold_fields_2 is not None:
            if holder[hold_fields_2["address_quantity"]] > 0 or (
                exclude_empty_holders == False
                and holder[hold_fields_2["address_quantity"]] == 0
            ):
                holders.append(
                    {
...
def holders(db, asset, exclude_empty_holders=False):
    ...
    holders += _get_holders(
        ...
        {
            "address": "source",
            "address_quantity": "give_remaining",
            "escrow": "tx_hash",
        },
        # exclude_empty_holders=exclude_empty_holders,
        table="orders",
    ...
    holders += _get_holders(
        ...
        {
            "address": "tx0_address",
            "address_quantity": "forward_quantity",
            "escrow": "id",
        },
        # exclude_empty_holders=exclude_empty_holders,
    ...
    holders += _get_holders(
        ...
        {
            "address": "tx1_address",
            "address_quantity": "backward_quantity",
            "escrow": "id",
        },
        # exclude_empty_holders=exclude_empty_holders,
    ...
    if asset == config.XCP:
        ...
        holders += _get_holders(
            ...
            {
                "address": "source",
                "address_quantity": "wager_remaining",
                "escrow": "tx_hash",
            },
            # exclude_empty_holders=exclude_empty_holders,
            table="bets",
