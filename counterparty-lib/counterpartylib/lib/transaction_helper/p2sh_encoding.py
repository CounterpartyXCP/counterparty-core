...
def decode_data_redeem_script(redeem_script, p2sh_is_segwit=False):
    ...
    if script_len == 41 and \
        ...
        redeem_script[35] == bitcoinlib.core.script.OP_CHECKSIGVERIFY and \
        redeem_script[37] == bitcoinlib.core.script.OP_DROP and \
                ...
                if redeem_script[pos] == bitcoinlib.core.script.OP_DROP:
                    ...
                    if type(opcode) != type(''):
                        if opcode >= bitcoinlib.core.script.OP_2 and opcode <= bitcoinlib.core.script.OP_15:
                            ...
                            pos += 1
                            valid_sig = redeem_script[pos] == bitcoinlib.core.script.OP_CHECKMULTISIGVERIFY
                        else:
                            # it's p2pkh
                            ...
                            valid_sig = redeem_script[pos] == bitcoinlib.core.script.OP_CHECKSIGVERIFY
                        pos += 1
                        ...
                        if valid_sig:
                            ...
                            redeem_script_is_valid = redeem_script[pos + 1 + unique_offfset_length] == bitcoinlib.core.script.OP_DROP and \
                                redeem_script[pos + 2 + unique_offfset_length] == bitcoinlib.core.script.OP_DEPTH and \
                                redeem_script[pos + 3 + unique_offfset_length] == 0 and \
                                redeem_script[pos + 4 + unique_offfset_length] == bitcoinlib.core.script.OP_EQUAL
...
def make_p2sh_encoding_redeemscript(datachunk, n, pub_key=None, multisig_pubkeys=None, multisig_pubkeys_required=None):
    ...
    cleanup_script = [n, bitcoinlib.core.script.OP_DROP, bitcoinlib.core.script.OP_DEPTH, 0, bitcoinlib.core.script.OP_EQUAL] # unique offset + prevent scriptSig malleability
    ...
    if pub_key is not None:
        ...
        verify_owner_script = verify_owner_script + [len(multisig_pubkeys), bitcoinlib.core.script.OP_CHECKMULTISIGVERIFY]
    else:
    ...
    _logger.debug(f'data_drop_script {repr(CScript(data_drop_script))} ({binascii.hexlify(CScript(data_drop_script))})')
    _logger.debug(f'verifyOwnerScript {repr(CScript(verify_owner_script))} ({binascii.hexlify(CScript(verify_owner_script))})')
