...
def calculate_outputs(destination_outputs, data_array, fee_per_kb, exact_fee=None):
    ...
    datatx_size += sum(
        [len(data_chunk) for data_chunk in data_array]
    )  # data in scriptSig
    ...
    if exact_fee:
        ...
        if remain_fee > 0:
            ...
            data_value[len(data_array) - 1] = (
                data_value[len(data_array) - 1] + remain_fee
            )
                    ...
                    if type(opcode) != type(""):
                        if (
                            ...
                            while not found_sigs:
                                ...
                                num_sigs += 1
                                if (
                                    redeem_script[pos] - bitcoinlib.core.script.OP_1 + 1
                                    == num_sigs
                                ):
                                    found_sigs = True
...
def make_p2sh_encoding_redeemscript(
    ...
    if pub_key is not None:
        ...
        raise exceptions.TransactionError(
            "Either pub_key or multisig pub_keys must be provided"
        )
    ...
    _logger.debug(
        f"entire redeem_script {repr(redeem_script)} ({binascii.hexlify(redeem_script)})"
    )
    ...
    _logger.debug(
        f"outputScript {repr(output_script)} ({binascii.hexlify(output_script)})"
    )
