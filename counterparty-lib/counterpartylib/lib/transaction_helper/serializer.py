...
D = decimal.Decimal
UTXO_LOCKS = None
UTXO_LOCKS_PER_ADDRESS_MAXSIZE = (
    5000  # set higher than the max number of UTXOs we should expect to
)
...
def make_fully_valid(pubkey_start):
    ...
    assert (
        len(pubkey_start) == 31
    )  # One sign byte and one nonce byte required (for 33 bytes).
...
def serialise(
    ...
    for data_chunk in data_array:
        ...
        if encoding == "multisig":
            ...
            tx_script += op_push(
                33
            )  # Push bytes of data chunk (fake) public key    (1/2)
            tx_script += data_pubkey_1  # (Fake) public key                  (1/2)
            tx_script += op_push(
                33
            )  # Push bytes of data chunk (fake) public key    (2/2)
            tx_script += data_pubkey_2  # (Fake) public key                  (2/2)
            tx_script += op_push(
                len(dust_return_pubkey)
            )  # Push bytes of source public key
            ...
            tx_script += op_push(
                len(data_chunk)
            )  # Push bytes of data chunk (NOTE: OP_SMALLDATA?)
...
def serialise_p2sh_pretx(
    ...
    for n, data_chunk in enumerate(data_array):
        ...
        # get the scripts
        script_sig, redeem_script, output_script = (
            p2sh_encoding.make_p2sh_encoding_redeemscript(
