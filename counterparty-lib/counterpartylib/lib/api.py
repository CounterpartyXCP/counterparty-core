...
JSON_RPC_ERROR_API_COMPOSE = (
    -32001
)  # code to use for error composing transaction result
...
def get_rows(
    ...
    for filter_ in filters:
        case_sensitive = (
            False if "case_sensitive" not in filter_ else filter_["case_sensitive"]
        )
...
class APIServer(threading.Thread):
    ...
    def run(self):
        ...
        def generate_create_method(tx):
            ...
            def create_method(**kwargs):
                try:
                    ...
                    error_msg = (
                        f"Error composing {tx} transaction via API: {str(error)}"
                    )
        ...
        def get_asset_info(assets=None, asset=None):
            ...
            for asset in assets:
                ...
                issuances = ledger.get_issuances(
                    self.db, asset=asset, status="valid", first=True
                )
                cursor.close()
                if not issuances:
        ...
        @dispatcher.add_method
        def fee_per_kb(
            conf_target=config.ESTIMATE_FEE_CONF_TARGET, mode=config.ESTIMATE_FEE_MODE
        ):
        ...
        def get_blocks(block_indexes, min_message_index=None):
            ...
            messages = collections.deque(
                ledger.get_messages(self.db, block_index_in=block_indexes)
            )

            # Discard any messages less than min_message_index
            if min_message_index:
                while (
                    len(messages) and messages[0]["message_index"] < min_message_index
                ):
                    messages.popleft()
            ...
            for block in blocks:
                block["_messages"] = []
                while (
                    len(messages) and messages[0]["block_index"] == block["block_index"]
                ):
        ...
        def get_running_info():
            ...
            try:
                ...
                indexd_blocks_behind = (
                    latest_block_index if latest_block_index > 0 else 999999
                )
        ...
        def get_unspent_txouts(
            address, unconfirmed=False, unspent_tx_hash=None, order_by=None
        ):
        ...
        def getrawtransaction(tx_hash, verbose=False, skip_missing=False):
            return backend.getrawtransaction(
        ...
        def get_dispenser_info(tx_hash=None, tx_index=None):
            ...
            if len(dispensers) == 1:
                ...
                if dispenser["oracle_address"] != None:
                    ...
                    if oracle_price > 0:
                        satoshi_price = math.ceil(
                            (fiat_price / oracle_price) * config.UNIT
                        )
                    else:
        ...
        def handle_rpc_post(request_json):
            ...
            try:
                ...
                return flask.Response(
                    obj_error.json.encode(), 400, mimetype="application/json"
                )
