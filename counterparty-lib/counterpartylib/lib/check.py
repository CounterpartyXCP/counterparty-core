    ...
    if not previous_consensus_hash:
        try:
            previous_consensus_hash = list(
                cursor.execute(
                    """SELECT * FROM blocks WHERE block_index = ?""", (block_index - 1,)
                )
            )[0][field]
        except IndexError:
...
def check_change(protocol_change, change_name):
    ...
    if not passed:
        explanation = (
            f"Your version of {config.APP_NAME} is v{config.VERSION_STRING}, but, "
        )
        explanation += (
            f"as of block {protocol_change['block_index']}, the minimum version is "
        )
...
def database_version(db):
    ...
    if version_major != config.VERSION_MAJOR:
        ...
        if config.NEED_REPARSE_IF_MINOR_IS_LESS_THAN is not None:
            min_version_minor, min_version_block_index = (
                config.NEED_REPARSE_IF_MINOR_IS_LESS_THAN
            )
