...
def test_array(signatures_required, pubs, signatures_possible):
    ...
    try:
        signatures_required, signatures_possible = int(signatures_required), int(
            signatures_possible
        )
...
def construct_array(signatures_required, pubs, signatures_possible):
    ...
    address = "_".join(
        [str(signatures_required)] + sorted(pubs) + [str(signatures_possible)]
    )
    return address
...
def extract_array(address):
    ...
    array = address.split("_")
    signatures_required, pubs, signatures_possible = (
        array[0],
        sorted(array[1:-1]),
        array[-1],
    )
...
def script_to_asm(scriptpubkey):
    try:
        script = (
            bytes(scriptpubkey, "utf-8")
            if type(scriptpubkey) == str
            else bytes(scriptpubkey)
        )
...
def make_pubkeyhash(address):
    ...
    if is_multisig(address):
        ...
        pubkeyhash_address = construct_array(
            signatures_required, pubkeyhashes, signatures_possible
        )
    else:
