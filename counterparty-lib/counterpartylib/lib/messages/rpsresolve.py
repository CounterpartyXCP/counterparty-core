...
def parse(db, tx, message):
    ...
    if status == "valid":
        txn, rps_match, problems = validate(
            db, tx["source"], move, random, rps_match_id
        )
        if problems:
            rps_match = None
    ...
    if status == "valid":
        ...
        if rps_match["status"] == "pending":
            rps_match_status = (
                "resolved and pending" if txn == 0 else "pending and resolved"
            )
        ...
        if rps_match_status == "concluded":
            ...
            if winner == 0:
                ...
                rps_match_status = (
                    f"concluded: {'first' if txn == 0 else 'second'} player wins"
                )
