...
def initialise(db):
    ...
    if database.has_fk_on(cursor, "bet_match_expirations", "bet_matches.id"):
        database.copy_old_table(
            cursor, "bet_match_expirations", create_bet_match_expirations_query
        )
    # create indexes
    database.create_indexes(
        cursor,
    ...
    if database.has_fk_on(cursor, "bet_match_resolutions", "bet_matches.id"):
        database.copy_old_table(
            cursor, "bet_match_resolutions", create_bet_match_resolutions_query
        )
...
def match(db, tx):
    ...
    if (
        ...
        sorted(
            bet_matches,
            key=lambda x: ledger.price(x["wager_quantity"], x["counterwager_quantity"]),
        )  # Sort by price first.
    ...
    for tx0 in bet_matches:
        ...
        if tx["block_index"] < 286000:
            tx0_inverse_odds = ledger.price(1, tx0_odds)  # Protocol change.

        logger.debug(
            f"Tx0 Inverse Odds: {float(tx0_inverse_odds)}; Tx1 Odds: {float(tx1_odds)}"
        )
        if tx0_inverse_odds > tx1_odds:
            logger.debug("Skipping: price mismatch.")
        else:
            logger.debug(
                f"Potential forward quantities: {tx0_wager_remaining}, {int(ledger.price(tx1_wager_remaining, tx1_odds))}"
            )
            forward_quantity = int(
                min(
                    tx0_wager_remaining,
                    int(ledger.price(tx1_wager_remaining, tx1_odds)),
                )
            )
            ...
            if not forward_quantity:
                ...
                continue
            if (
                tx1["block_index"] >= 286500 or config.TESTNET or config.REGTEST
            ):  # Protocol change.
            ...
            if (
                tx1["block_index"] >= 292000 or config.TESTNET or config.REGTEST
            ):  # Protocol change
...
def expire(db, block_index, block_time):
    ...
    for bet_match in ledger.get_bet_matches_to_expire(db, block_time):
        ...
        ledger.insert_record(
            db, "bet_match_expirations", bindings, "BET_MATCH_EXPIRATION"
        )
