...
def validate(db, source, timestamp, value, fee_fraction_int, text, block_index):
    ...
    # For SQLite3
    if (
        timestamp > config.MAX_INT
        or value > config.MAX_INT
        or fee_fraction_int > config.MAX_INT
    ):
    ...
    if (
        ledger.enabled("options_require_memo")
        and text
        and text.lower().startswith("options")
    ):
        try:
        ...
        if value == -3:
            for bet_match in ledger.get_pending_bet_matches(db, tx["source"]):
                bet.cancel_bet_match(
                    db, bet_match, "dropped", tx["block_index"], tx["tx_index"]
                )
        cursor.close()
        return
