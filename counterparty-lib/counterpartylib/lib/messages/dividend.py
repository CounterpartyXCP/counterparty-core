...
def validate(db, source, quantity_per_unit, asset, dividend_asset, block_index):
    ...
    for holder in holders:
        if block_index < 294500 and not (
            config.TESTNET or config.REGTEST
        ):  # Protocol change.
            if holder["escrow"]:
                continue
        ...
        if (
            block_index >= 296000 or config.TESTNET or config.REGTEST
        ):  # Protocol change.
            if address == source:
                continue
        ...
        if (
            dividend_asset == config.BTC
            and dividend_quantity < config.DEFAULT_MULTISIG_DUST_SIZE
        ):
    ...
    if not problems and dividend_asset != config.BTC:
        ...
        if (
            block_index >= 330000 or config.TESTNET or config.REGTEST
        ):  # Protocol change.
...
def compose(db, source, quantity_per_unit, asset, dividend_asset):
    ...
    dividend_asset_id = ledger.get_asset_id(
        db, dividend_asset, ledger.CURRENT_BLOCK_INDEX
    )
...
def parse(db, tx, message):
    ...
    try:
        if (tx["block_index"] > 288150 or config.TESTNET or config.REGTEST) and len(
            ...
            quantity_per_unit, asset_id, dividend_asset_id = struct.unpack(
                FORMAT_2, message
            )
            asset = ledger.get_asset_name(db, asset_id, tx["block_index"])
            dividend_asset = ledger.get_asset_name(
                db, dividend_asset_id, tx["block_index"]
            )
            status = "valid"
    ...
    if status == "valid":
        ...
        if (
            tx["block_index"] >= 330000 or config.TESTNET or config.REGTEST
        ):  # Protocol change.
            ledger.debit(
                db,
                tx["source"],
        ...
        for output in outputs:
            if (
                not ledger.enabled("dont_credit_zero_dividend")
                or output["dividend_quantity"] > 0
            ):
                ledger.credit(
                    db,
                    output["address"],
