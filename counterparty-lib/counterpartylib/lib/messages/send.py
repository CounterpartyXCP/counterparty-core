...
def initialise(db):
    ...
    columns = [
        column["name"] for column in cursor.execute("""PRAGMA table_info(sends)""")
    ]
    ...
    if "msg_index" not in columns:
        if "memo" not in columns:
            cursor.execute(
                """CREATE TABLE IF NOT EXISTS new_sends(
                              tx_index INTEGER,
                              tx_hash TEXT,
                              block_index INTEGER,
                              source TEXT,
...
def compose(
    ...
    if ledger.enabled("enhanced_sends"):
        ...
        if (
            ...
            and isinstance(asset, list)
            and isinstance(quantity, list)
        ):
                ...
                if len(destination) == len(asset) and len(asset) == len(quantity):
                    ...
                    if isinstance(memo, list) and isinstance(memo_is_hex, list):
                        ...
                        return mpma.compose(
                            ...
                            source,
                            util.flat(
                                zip(asset, destination, quantity, memo, memo_is_hex)
                            ),
                            None,
                            None,
                        )
