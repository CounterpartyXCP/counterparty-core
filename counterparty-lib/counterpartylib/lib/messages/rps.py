...
def initialise(db):
    ...
    if database.has_fk_on(cursor, "rps_match_expirations", "rps_matches.id"):
        database.copy_old_table(
            cursor, "rps_match_expirations", create_rps_match_expirations_query
        )
    # create indexes
    database.create_indexes(
        cursor,
...
def validate(
    db, source, possible_moves, wager, move_random_hash, expiration, block_index
):
    problems = []
...
def parse(db, tx, message):
    ...
    try:
        ...
        (possible_moves, wager, move_random_hash, expiration) = struct.unpack(
            FORMAT, message
        )
        status = "open"
...
def match(db, tx, block_index):
    ...
    rps_matches = ledger.get_matching_rps(
        db, possible_moves, wager, tx1["source"], already_matched
    )
...
def expire(db, block_index):
    ...
    for rps_match in ledger.get_rps_matches_to_expire(db, block_index):
        ...
        ledger.insert_record(
            db, "rps_match_expirations", bindings, "RPS_MATCH_EXPIRATION"
        )
