...
def initialise(db):
    ...
    if database.has_fk_on(cursor, "order_expirations", "orders.tx_index"):
        database.copy_old_table(
            cursor, "order_expirations", create_order_expirations_query
        )
    # create indexes
    database.create_indexes(
        cursor,
...
def exact_penalty(db, address, block_index, order_match_id, tx_index):
    ...
    if not (
        block_index >= 314250 or config.TESTNET or config.REGTEST
    ):  # Protocol change.
        # Order matches.
...
def cancel_order_match(db, order_match, status, block_index, tx_index):
    ...
    if tx0_order["status"] in ("expired", "cancelled"):
        ...
        tx0_give_remaining = (
            tx0_order["give_remaining"] + order_match["forward_quantity"]
        )
        tx0_get_remaining = (
            tx0_order["get_remaining"] + order_match["backward_quantity"]
        )
        ...
        if tx0_order_status == "filled" and ledger.enabled(
            "reopen_order_when_btcpay_expires_fix", block_index
    ...
    if tx1_order["status"] in ("expired", "cancelled"):
        ...
        tx1_give_remaining = (
            tx1_order["give_remaining"] + order_match["backward_quantity"]
        )
        ...
        tx1_order_status = tx1_order["status"]
        if tx1_order_status == "filled" and ledger.enabled(
            "reopen_order_when_btcpay_expires_fix", block_index
        ):  # This case could happen if a BTCpay expires and before the expiration, the order was filled by a correct BTCpay
    ...
    if block_index >= 313900 or config.TESTNET or config.REGTEST:  # Protocol change.
        if (
            tx0_order["status"] == "expired"
            and order_match["forward_asset"] == config.BTC
        ):
            exact_penalty(
                db, order_match["tx0_address"], block_index, order_match["id"], tx_index
            )
        if (
            tx1_order["status"] == "expired"
            and order_match["backward_asset"] == config.BTC
        ):
            exact_penalty(
                db, order_match["tx1_address"], block_index, order_match["id"], tx_index
            )

    # Re‐match.
    if block_index >= 310000 or config.TESTNET or config.REGTEST:  # Protocol change.
        if not (
            block_index >= 315000 or config.TESTNET or config.REGTEST
        ):  # Protocol change.
            match(
                db,
                ledger.get_transactions(db, tx_hash=tx0_order["tx_hash"])[0],
                block_index,
            )
            match(
                db,
                ledger.get_transactions(db, tx_hash=tx1_order["tx_hash"])[0],
                block_index,
            )
...
def parse(db, tx, message):
    ...
    try:
        ...
        give_id, give_quantity, get_id, get_quantity, expiration, fee_required = (
...
def match(db, tx, block_index=None):
    ...
    if (
        tx["block_index"] > 284500 or config.TESTNET or config.REGTEST
    ):  # Protocol change.
        ...
        order_matches = sorted(
            order_matches,
            key=lambda x: ledger.price(x["get_quantity"], x["give_quantity"]),
        )  # Sort by price first.
    ...
    for tx0 in order_matches:
        ...
        if (
            tx0["give_asset"] == config.BTC or tx0["get_asset"] == config.BTC
        ):  # Gratuitous
            ...
            if (
                block_index >= 294000 or config.TESTNET or config.REGTEST
            ):  # Protocol change.
        ...
        if tx0_price > tx1_inverse_price:
            ...
            forward_quantity = int(
                min(
                    tx0_give_remaining, int(ledger.price(tx1_give_remaining, tx0_price))
                )
            )
            ...
            if (
                ...
                min_btc_quantity = 0.001 * config.UNIT  # 0.001 BTC
                if (
                    forward_asset == config.BTC and forward_quantity <= min_btc_quantity
                ) or (
            ...
            if (
                block_index >= 308000 or config.TESTNET or config.REGTEST
            ):  # Protocol change.
...
def expire(db, block_index):
    ...
    for order in orders:
        cancel_order(
            db, order, "expired", block_index, 0
        ...
        if ledger.enabled("btc_sell_expire_on_match_expire"):
            ...
            if len(order_matches_pending) == 0:
                if (
                    ...
                    and order_match["status"] == "expired"
                ):
                    cancel_order(
                        db,
