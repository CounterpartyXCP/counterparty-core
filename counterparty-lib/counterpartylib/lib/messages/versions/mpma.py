...
def validate(db, source, asset_dest_quant_list, block_index):
    ...
    for t in asset_dest_quant_list:
        ...
        if not isinstance(quantity, int):
            problems.append(
                f"quantities must be an int (in satoshis) for {asset} to {destination}"
            )
        ...
        if ledger.enabled("options_require_memo"):
            results = (
                ledger.get_addresses(db, address=destination) if destination else None
            )
            if results:
                result = results[0]
                if (
...
def parse(db, tx, message):
    ...
    if status == "valid":
        for asset_id in unpacked:
            ...
            if status == "valid":
                plain_sends += map(
                    lambda t: util.py34_tuple_append(asset_id, t), credits
                )
                all_credits += map(
                    lambda t: {
                        "asset": asset_id,
