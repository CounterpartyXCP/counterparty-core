...
LENGTH = 22
MAX_MEMO_LENGTH = (
    34  # Could be higher, but we will keep it consistent with enhanced send
)
ID = 4
...
def validate(db, source, destination, flags, memo, block_index):
    ...
    antispamfee = (
        ledger.get_value_by_block_index("sweep_antispam_fee", block_index) * config.UNIT
    )
    ...
    if antispamfee > 0:
        ...
        total_fee = int(
            balances_count * antispamfee * 2 + issuances_count * antispamfee * 4
        )
                ...
                if len(issuances) > 0:
                    ...
                    if last_issuance["issuer"] == tx["source"]:
                        ...
                        ledger.insert_record(
                            db, "issuances", bindings, "ASSET_TRANSFER"
                        )
                        sweep_pos += 1
