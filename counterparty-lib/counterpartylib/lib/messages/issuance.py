...
def initialise(db):
    ...
    columns = [
        column["name"] for column in cursor.execute("""PRAGMA table_info(issuances)""")
    ]
...
def validate(
    ...
    if not callable_:
        if (
            block_index >= 312500 or config.TESTNET or config.REGTEST
        ):  # Protocol change.
            call_date = 0
            call_price = 0.0
    ...
    if issuances:
        ...
        if (
            not ledger.enabled("issuance_callability_parameters_removal", block_index)
        ...
        if last_issuance["call_date"] > call_date and (
            call_date != 0
            or (block_index < 312500 and (not config.TESTNET or not config.REGTEST))
        ):
    ...
    if quantity or (
        ...
        if not reissuance or (
            ...
            if ledger.enabled("numeric_asset_names"):  # Protocol change.
                ...
                block_index >= 291700 or config.TESTNET or config.REGTEST
            ):  # Protocol change.
                fee = int(0.5 * config.UNIT)
            elif (
                block_index >= 286000 or config.TESTNET or config.REGTEST
            ):  # Protocol change.
                fee = 5 * config.UNIT
            elif (
                block_index > 281236 or config.TESTNET or config.REGTEST
            ):  # Protocol change.
                fee = 5
    ...
    if reset and ledger.enabled(
        "cip03", block_index
    ):  # reset will overwrite the quantity
...
def compose(
    ...
    if subasset_longname is None or reissuance:
        ...
        if description == None and ledger.enabled("issuance_description_special_null"):
            ...
            encoded_description = (
                DESCRIPTION_MARK_BYTE + DESCRIPTION_NULL_ACTION.encode("utf-8")
            )
        else:
            if (len(validated_description) <= 42) and not ledger.enabled(
    ...
    try:
        ...
        if message_type_id == LR_SUBASSET_ID or message_type_id == SUBASSET_ID:
            if not ledger.enabled("subassets", block_index=tx["block_index"]):
                logger.warning(
                    f"subassets are not enabled at block {tx['block_index']}"
                )
            ...
            if subasset_format_length <= 18:
                asset_id, quantity, divisible, compacted_subasset_length = (
                    struct.unpack(subasset_format, message[0:subasset_format_length])
                )
            elif subasset_format_length <= 19:  # param reset was inserted
                asset_id, quantity, divisible, reset, compacted_subasset_length = (
                    struct.unpack(subasset_format, message[0:subasset_format_length])
                )
            elif subasset_format_length <= 20:  # param lock was inserted
                (
            ...
            try:
                ...
                if description_data[0:1] == DESCRIPTION_MARK_BYTE:
                    try:
                        if (
                            description_data[1:].decode("utf-8")
                            == DESCRIPTION_NULL_ACTION
                        ):
                            description = None
                    except UnicodeDecodeError:
                        description = ""
            ...
            if owner_address == tx["source"]:
                if owner_balance > 0:
                    ...
                    ledger.insert_record(
                        db, "destructions", bindings, "ASSET_DESTRUCTION"
                    )
