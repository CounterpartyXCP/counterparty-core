...
def initialise(db):
    ...
    columns = [
        column["name"] for column in cursor.execute("""PRAGMA table_info(dispensers)""")
    ]
    ...
    if "dispense_count" not in columns:
        cursor.execute(
            "ALTER TABLE dispensers ADD COLUMN dispense_count INTEGER DEFAULT 0"
        )
    ...
    # add new columns if not exist
    columns = [
        column["name"] for column in cursor.execute("""PRAGMA table_info(dispenses)""")
    ]
...
def validate(
    ...
    if len(available) == 0:
        ...
        if (
            ...
            query_address = (
                open_address if status == STATUS_OPEN_EMPTY_ADDRESS else source
            )
            open_dispensers = ledger.get_dispensers(
        ...
        if len(open_dispensers) == 0 or open_dispensers[0]["status"] != STATUS_CLOSING:
            if status == STATUS_OPEN or status == STATUS_OPEN_EMPTY_ADDRESS:
                if len(open_dispensers) > 0:
                    max_refills = ledger.get_value_by_block_index(
                        "max_refills", block_index
                    )
                    refilling_count = 0
                    if max_refills > 0:
                    ...
                    if (
                        ...
                        if (max_refills > 0) and (refilling_count >= max_refills):
                            problems.append(
                                "the dispenser reached its maximum refilling"
                            )
                    else:
                ...
                if len(open_dispensers) == 0:
                    problems.append(
                        f"address doesnt has an open dispenser for asset {asset}"
                    )
            ...
            if status == STATUS_OPEN_EMPTY_ADDRESS:
                ...
                if not (
                    ...
                    if not (
                        ledger.enabled(
                            "dispenser_origin_permission_extended", block_index
                        )
                        ...
                        if ledger.enabled(
                            ...
                            if (
                                ...
                                and (
                                    block_index
                                    > address_oldest_transaction["block_index"]
                                )
                            ):
                                problems.append(
...
def compose(
    ...
    data += struct.pack(
        FORMAT, assetid, give_quantity, escrow_quantity, mainchainrate, status
    )
...
def parse(db, tx, message):
    ...
    try:
        ...
        oracle_address = None
        assetid, give_quantity, escrow_quantity, mainchainrate, dispenser_status = (
            struct.unpack(FORMAT, message[0:LENGTH])
        )
        read = LENGTH
    ...
    if status == "valid":
        ...
        if problems:
            ...
            if (
                dispenser_status == STATUS_OPEN
                or dispenser_status == STATUS_OPEN_EMPTY_ADDRESS
            ):
                ...
                if len(existing) == 0:
                    if (oracle_address != None) and ledger.enabled(
                        ...
                        if oracle_fee >= config.DEFAULT_REGULAR_DUST_SIZE:
                            if (
                                ...
                                status = (
                                    "invalid: insufficient or non-existent oracle fee"
                                )
                    ...
                    if status == "valid":
                        ...
                        try:
                            if dispenser_status == STATUS_OPEN_EMPTY_ADDRESS:
                                ...
                                address_assets = ledger.get_address_assets(
                                    db, action_address
                                )
                    ...
                    if status == "valid":
                        ...
                        ledger.insert_record(
                            db, "dispensers", bindings, "OPEN_DISPENSER"
                        )
                elif (
                    len(existing) == 1
                    ...
                    if tx["source"] == action_address or (
                        ledger.enabled(
                            "dispenser_origin_permission_extended", tx["block_index"]
                        )
                        ...
                        status = (
                            "invalid: can only refill dispenser from source or origin"
                        )
                else:
        ...
        for dispenser in dispensers:
            ...
            if satoshirate > 0 and give_quantity > 0:
                if (dispenser["oracle_address"] != None) and ledger.enabled(
                    ...
                    must_give = int(
                        floor(
                            ((next_out["btc_amount"] / config.UNIT) * last_price)
                            / fiatrate
                        )
                    )
                else:
                ...
                if (
                    ledger.enabled("zero_quantity_value_adjustment_1")
                    and actually_given == 0
                ):
                    continue
                ...
                if (
                    give_remaining < dispenser["give_quantity"]
                    or max_dispenser_limit_hit
                ):
                    # close the dispenser
