    ...
    if custom_inputs:
        ...
        for output in unspent:
            if (
                ...
                and make_outkey_vin_txid(output["txid"], output["vout"])
        ...
        logger.debug(
            f"Sorted candidate UTXOs: {[print_coin(coin) for coin in unspent]}"
        )
        use_inputs = unspent

    # use backend estimated fee_per_kb
    if estimate_fee_per_kb:
        estimated_fee_per_kb = backend.fee_per_kb(
...
def select_any_coin_from_source(
    source, allow_unconfirmed_inputs=True, disable_utxo_locks=False
):
    ...
    # sort
    unspent = backend.sort_unspent_txouts(
...
def construct(
    ...
    if data:
        ...
        if encoding == "multisig":
            ...
            if (source_is_p2sh and dust_return_pubkey is None) or (
                dust_return_pubkey is False and config.P2SH_DUST_RETURN_PUBKEY is None
            ):
    ...
    outputs_size = (p2pkhsize * len(destination_outputs)) + (
        len(data_array) * data_output_size
    )
    ...
    if encoding == "p2sh":
        ...
        if segwit:
            ...
            pretx_txid = bytes(
                txid_ba
            )  # gonna leave the malleability problem to upstream
    ...
    try:
        ...
        return return_result(
            [unsigned_pretx_hex, unsigned_tx_hex], old_style_api=old_style_api
        )
    desired_source = script.make_canonical(desired_source)
