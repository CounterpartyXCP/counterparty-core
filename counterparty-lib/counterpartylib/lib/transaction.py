    ...
    if custom_inputs:
        ...
        if unspent_tx_hash is not None:
            unspent = backend.get_unspent_txouts(
                source, unconfirmed=allow_unconfirmed_inputs, unspent_tx_hash=unspent_tx_hash
            )
        else:
    ...
    if data:
        ...
        if desired_encoding == "auto":
            if len(data) + len(config.PREFIX) <= config.OP_RETURN_MAX_SIZE:
                ...
                encoding = (
                    "p2sh" if not old_style_api and ledger.enabled("p2sh_encoding") else "multisig"
        ...
        for address, value in destination_outputs:
            ...
            if script.is_multisig(address):
                destination_outputs_new.append(
                    (backend.multisig_pubkeyhashes_to_pubkeys(address, provided_pubkeys), value)
                )
            else:
