...
def parse_tx(db, tx):
    ...
    try:
        with db:
            ...
            # Protocol change.
            rps_enabled = (
                tx["block_index"] >= 308500 or config.TESTNET or config.REGTEST
            )
            ...
            if message_type_id == send.ID:
                ...
                ledger.enabled(
                    "issuance_backwards_compatibility", block_index=tx["block_index"]
                )
                ...
                if tx["block_index"] != config.MEMPOOL_BLOCK_INDEX:
                    logger.info(
                        f"Unsupported transaction: hash {tx['tx_hash']}; data {tx['data']}"
                    )
                cursor.close()
                return False
...
def initialise(db):
    ...
    block_columns = [
        column["name"] for column in cursor.execute("""PRAGMA table_info(blocks)""")
    ]
    ...
    cursor.execute(
        """DELETE FROM blocks WHERE block_index < ?""", (config.BLOCK_FIRST,)
    )
    cursor.execute(
        """DELETE FROM transactions WHERE block_index < ?""", (config.BLOCK_FIRST,)
    )
    ...
    debits_columns = [
        column["name"] for column in cursor.execute("""PRAGMA table_info(debits)""")
    ]
    ...
    #   SQLite can’t do `ALTER TABLE IF COLUMN NOT EXISTS`.
    columns = [
        column["name"] for column in cursor.execute("""PRAGMA table_info(assets)""")
    ]
    ...
    if not list(cursor):
        cursor.execute(
            """INSERT INTO assets VALUES (?,?,?,?)""", ("0", "BTC", None, None)
        )
        cursor.execute(
            """INSERT INTO assets VALUES (?,?,?,?)""", ("1", "XCP", None, None)
        )
    ...
    columns = [
        column["name"] for column in cursor.execute("""PRAGMA table_info(messages)""")
    ]
...
def list_tx(
    ...
    if source and (data or destination == config.UNSPENDABLE or dispensers_outs):
        ...
        ledger.insert_record(
            db, "transactions", transaction_bindings, "NEW_TRANSACTION"
        )
...
def clean_table_from(cursor, table, block_index):
    ...
    cursor.execute(
        f"""DELETE FROM {table} WHERE block_index >= ?""", (block_index,)
    )  # nosec B608
...
def generate_progression_message(
    ...
    current_block = (
        f"Block {block['block_index']} parsed in {block_parsing_duration:.3f}s"
    )
    ...
    while True:
        ...
        if block_index <= block_count:
            ...
            if current_index in prefetcher.BLOCKCHAIN_CACHE:
                ...
                raw_transactions = prefetcher.BLOCKCHAIN_CACHE[current_index][
                    "raw_transactions"
                ]
                ...
                block_difficulty = prefetcher.BLOCKCHAIN_CACHE[current_index][
                    "block_difficulty"
                ]
                ...
                if block_index < block_count - 100:
                    logger.warning(
                        f"Blockchain cache miss :/ Block index: {current_index}"
                    )
            ...
            try:
                raw_transactions = backend.getrawtransaction_batch(
                    parse_txs, skip_missing=True
                )
            except Exception as e:
                logger.warning(
                    "Failed to fetch raw for mempool TXs, restarting loop; %s", (e,)
                )
            ...
            for tx_hash in parse_txs:
                ...
                if parsed_txs_count % 100 == 0:
                    ...
                    try:
                        ...
                        if block_index <= block_count:
                            logger.info(
                                "Mempool parsing interrupted, there are blocks to parse"
                            )
