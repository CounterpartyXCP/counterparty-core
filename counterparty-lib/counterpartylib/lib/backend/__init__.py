...
def cache_pretx(txid, rawtx):
    PRETX_CACHE[binascii.hexlify(txid).decode("utf8")] = binascii.hexlify(rawtx).decode(
        "utf8"
    )
...
def ensure_script_pub_key_for_inputs(coins):
    ...
    if len(txhash_set) > 0:
        txs = backend().getrawtransaction_batch(
...
def sort_unspent_txouts(
    unspent, unconfirmed=False, dust_size=config.DEFAULT_REGULAR_DUST_SIZE
):
    ...
    if config.REGTEST:
        ...
        unspent = sorted(
            unspent, key=lambda x: (x["confirmations"], x["value"]), reverse=True
        )
    else:
            ...
            if "txinwitness" in vin:
                if len(vin["txinwitness"]) >= 2:
                    ...
                    try:
                        ...
                        if pubkeyhash == script.pubkey_to_p2whash(
