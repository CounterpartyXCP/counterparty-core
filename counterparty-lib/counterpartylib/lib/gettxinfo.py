...
def decode_checksig(asm, decoded_tx):
    ...
    if chunk[1 : len(config.PREFIX) + 1] == config.PREFIX:  # Data
        ...
        destination, data = (
            script.base58_check_encode(pubkeyhash, config.ADDRESSVERSION),
            None,
        )
    return destination, data
...
def decode_checkmultisig(asm, decoded_tx):
    ...
    if chunk[1 : len(config.PREFIX) + 1] == config.PREFIX:  # Data
        ...
        destination, data = (
            script.construct_array(
                signatures_required, pubkeyhashes, len(pubkeyhashes)
            ),
            None,
        )
...
def get_tx_info_new(
    db, decoded_tx, block_index, block_parser=None, p2sh_is_segwit=False
):
    ...
    if "parsed_vouts" in decoded_tx:
        if decoded_tx["parsed_vouts"] == "DecodeError":
            raise DecodeError("unrecognised output type")
        destinations, btc_amount, fee, data, potential_dispensers = decoded_tx[
            "parsed_vouts"
        ]
    else:
        destinations, btc_amount, fee, data, potential_dispensers = (
    ...
    if p2sh_encoding_source is None:
        sources, outputs_value = get_transaction_sources(
            decoded_tx, block_parser=block_parser
        )
        if not fee_added:
            fee += outputs_value
...
def get_tx_info_legacy(decoded_tx, block_index, block_parser=None):
    ...
    for vout in decoded_tx["vout"]:
        ...
        if len(asm) == 2 and asm[0] == OP_RETURN:  # OP_RETURN
            ...
            # Be strict.
            pubkeyhash, address_version = get_pubkeyhash(
                vout["scriptPubKey"], block_index
            )
            if not pubkeyhash:
                continue
...
def _get_tx_info(db, decoded_tx, block_index, block_parser=None, p2sh_is_segwit=False):
    ...
    if ledger.enabled("p2sh_addresses", block_index=block_index):  # Protocol change.
        ...
        "multisig_addresses", block_index=block_index
    ):  # Protocol change.
        return get_tx_info_new(
            db,
            decoded_tx,
