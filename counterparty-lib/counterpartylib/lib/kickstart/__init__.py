...
from counterpartylib.lib.kickstart.blocks_parser import (
    BlockchainParser,
    ChainstateParser,
)
...
def confirm_kickstart():
    ...
    confirmation_message = colored(
        "Proceed with the initialization? (y/N): ", "magenta"
    )
...
def fetch_blocks(cursor, bitcoind_dir, last_known_hash, first_block, spinner):
    ...
    cursor.execute(
        """CREATE TABLE IF NOT EXISTS kickstart_blocks (
    ...
    try:
        ...
        block_parser.close()
        spinner.text = (
            f"Blocks indexed in: {time.time() - start_time_blocks_indexing:.3f}s"
        )
        return block_count
...
def prepare_db_for_resume(cursor):
    ...
    cursor.execute(
        """SELECT block_index, tx_index FROM transactions ORDER BY block_index DESC, tx_index DESC LIMIT 1"""
    )
...
def get_bitcoind_dir(bitcoind_dir=None):
    ...
    if not os.path.isdir(bitcoind_dir):
        raise Exception(
            f"Bitcoin Core data directory not found at {bitcoind_dir}. Use --bitcoind-dir parameter."
        )
    if config.TESTNET:
    ...
    with Halo(text=step, spinner=SPINNER_STYLE) as spinner:
        ...
        if not resuming:
            ...
            if last_block is not None:
                ...
                first_kickstart_block = cursor.execute(
                    "SELECT block_index FROM kickstart_blocks ORDER BY block_index LIMIT 1"
                ).fetchone()["block_index"]
...
def start_blocks_parser_process(bitcoind_dir, last_parsed_block, max_queue_size):
    ...
    with Halo(text=step, spinner=SPINNER_STYLE):
        ...
        queue_size = (
            max_queue_size if max_queue_size is not None else default_queue_size
        )
        # Start block parser.
...
def backup_if_needed(new_database, resuming):
    if not new_database and not resuming:
        ...
        if version_major < 10:
            ...
            print(
                colored(
                    f"Old database will be moved to `{config.DATABASE}.old` and a new database will be created from scratch.",
                    "yellow",
                )
            )
...
def run(bitcoind_dir, force=False, max_queue_size=None, debug_block=None):
    ...
    # Start block parser.
    block_parser = start_blocks_parser_process(
        bitcoind_dir, last_parsed_block, max_queue_size
    )
