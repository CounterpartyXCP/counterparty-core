...
def set_options (data_dir=None,
    ...
    # no_interaction (skip prompting user for confirmation)
    if no_interaction:
        config.NO_INTERACTION = no_interaction
    elif has_config and 'no_interaction' in configfile['Default']:
        config.NO_INTERACTION = configfile['Default'].getboolean('no_interaction')
    else:
        config.NO_INTERACTION = False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-interaction', action='store_true', default=False, help='do not request user confirmation before signing and broadcasting')
