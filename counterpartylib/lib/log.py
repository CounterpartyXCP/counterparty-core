...
class NodeJSStyleLoggingFilter(logging.Filter):
    """
    ...
    def filter(self, record):
        ...
        for filter in self.filters:
            if filter[:1] == "-":
                if result is None and NodeJSStyleLoggingFilter.ismatch(record, filter[1:]):
                    result = False
            else:
                if NodeJSStyleLoggingFilter.ismatch(record, filter):
                    result = True
...
def set_up(logger, verbose=False, logfile=None, console_logfilter=None):
    ...
    if console_logfilter:
        console.addFilter(NodeJSStyleLoggingFilter(console_logfilter))
