...
def _transfer_funds(server_db, dispatcher, src_wif,
                    dest_address, asset, netcode):

    def get_txs(txids):
        return dispatcher.get("getrawtransaction_batch")(txhash_list=txids)

    src_address = address_from_wif(src_wif)
    unsigned_rawtx = dispatcher.get("create_send")(
        source=src_address,
    ...
    return util_test.insert_raw_transaction(signed_rawtx, server_db)


class _MockDispatcher(object):

    def get(self, name):
        def wrapper(**kwargs):
            return util.api(name, kwargs)
        return wrapper


@pytest.fixture(scope='function')
def mpcenv(server_db):
    ...
    # actors
    alice_wif = BIP32Node.from_master_secret(b"alice", netcode=netcode).wif()
    alice_address = address_from_wif(alice_wif)
    alice_pubkey = pubkey_from_wif(alice_wif)
    bob_wif = BIP32Node.from_master_secret(b"bob", netcode=netcode).wif()
    bob_address = address_from_wif(bob_wif)
    bob_pubkey = pubkey_from_wif(bob_wif)

    # create and fund alice wif (transaction must be signed)
    alice_src_wif = DEFAULT_PARAMS["addresses"][0][2]
    _transfer_funds(server_db, dispatcher, alice_src_wif,
                    alice_address, asset, netcode)

    # create and fund bob wif (transaction must be signed)
    bob_src_wif = DEFAULT_PARAMS["addresses"][1][2]
    _transfer_funds(server_db, dispatcher, bob_src_wif,
                    bob_address, asset, netcode)
