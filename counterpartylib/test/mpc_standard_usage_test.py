...
def test_usage_xcp(server_db, mpcenv):
    ...
    commit_address = script_address(highest_commit["script"],
                                    netcode=mpcenv["netcode"])
    alice_balance = util.get_balance(server_db, mpcenv["alice_address"],
                                     mpcenv["asset"])
    commit_balance = util.get_balance(server_db, commit_address,
                                      mpcenv["asset"])
    bob_balance = util.get_balance(server_db, mpcenv["bob_address"],
                                   mpcenv["asset"])
    assert alice_balance == 1000000 - deposit_quantity
    ...
    # let delay time pass
    for i in range(mpcenv["delay_time"] - 1):
    ...
    signed_payout_rawtx = scripts.sign_payout_recover(
        get_txs, mpcenv["bob_wif"], payout["payout_rawtx"],
        commit_script, mpcenv["spend_secret"]
    )
    ...
    alice_balance = util.get_balance(server_db, mpcenv["alice_address"],
                                     mpcenv["asset"])
    assert alice_balance == 1000000 - 17
