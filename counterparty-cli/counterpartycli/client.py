...
CONFIG_ARGS = [
    ...
    [('--fee-per-kb',), {'type': D, 'default': D(config.DEFAULT_FEE_PER_KB / config.UNIT), 'help': f'fee per kilobyte, in {config.BTC}'}],
    [('--regular-dust-size',), {'type': D, 'default': D(config.DEFAULT_REGULAR_DUST_SIZE / config.UNIT), 'help': f'value for dust Pay‐to‐Pubkey‐Hash outputs, in {config.BTC}'}],
    [('--multisig-dust-size',), {'type': D, 'default': D(config.DEFAULT_MULTISIG_DUST_SIZE / config.UNIT), 'help': f'for dust OP_CHECKMULTISIG outputs, in {config.BTC}'}],
    [('--op-return-value',), {'type': D, 'default': D(config.DEFAULT_OP_RETURN_VALUE / config.UNIT), 'help': f'value for OP_RETURN outputs, in {config.BTC}'}],
...
def main():
    ...
    parser_dispenser.add_argument('--mainchainrate', required=True, help=f'the quantity of {config.BTC} (decimal) this dispenser must receive to send the GIVEN_QUANTITY of the ASSET')
    parser_dispenser.add_argument('--give-quantity', required=True, help=f'the quantity of ASSET that you are giving for each MAINCHAINRATE of {config.BTC} received')
    ...
    parser_order.add_argument('--fee-fraction-required', default=config.DEFAULT_FEE_FRACTION_REQUIRED, help=f'the miners’ fee required for an order to match this one, as a fraction of the {config.BTC} to be bought')
    parser_order_fees = parser_order.add_mutually_exclusive_group()
    parser_order_fees.add_argument('--fee-fraction-provided', default=config.DEFAULT_FEE_FRACTION_PROVIDED, help=f'the miners’ fee provided, as a fraction of the {config.BTC} to be sold')
    parser_order_fees.add_argument('--fee', help=f'the exact {config.BTC} fee to be paid to miners')

    parser_btcpay = subparsers.add_parser(f'{config.BTC}pay'.lower(), help=f'create and broadcast a *{config.BTC}pay* message, to settle an Order Match for which you owe {config.BTC}')
    ...
    parser_bet.add_argument('--bet-type', choices=list(BET_TYPE_NAME.values()), required=True, help=f'choices: {list(BET_TYPE_NAME.values())}')
    ...
    parser_burn.add_argument('--quantity', required=True, help=f'quantity of {config.BTC} to be burned')
    ...
    parser_publish.add_argument('--startgas', required=True, type=int, help=f'the maximum quantity of {config.XCP} to be used to pay for the execution (satoshis)')
    parser_publish.add_argument('--endowment', required=True, type=int, help=f'quantity of {config.XCP} to be transfered to the contract (satoshis)')
    ...
    parser_execute.add_argument('--startgas', required=True, type=int, help=f'the maximum quantity of {config.XCP} to be used to pay for the execution (satoshis)')
    parser_execute.add_argument('--value', required=True, type=int, help=f'quantity of {config.XCP} to be transfered to the contract (satoshis)')
    ...
    parser_address = subparsers.add_parser('balances', help=f'display the balances of a {config.XCP_NAME} address')
    parser_address.add_argument('address', help='the address you are interested in')

    parser_asset = subparsers.add_parser('asset', help=f'display the basic properties of a {config.XCP_NAME} asset')
    parser_asset.add_argument('asset', help='the asset you are interested in')

    parser_wallet = subparsers.add_parser('wallet', help=f'list the addresses in your backend wallet along with their balances in all {config.XCP_NAME} assets')

    parser_pending = subparsers.add_parser('pending', help=f'list pending order matches awaiting {config.BTC}payment from you')
        ...
        if not args.unsigned:
            if script.is_multisig(args.source):
                ...
                if wallet.is_mine(args.source):
                    ...
                    private_key_wif = input(f'Source address not in wallet. Please enter the private key in WIF format for {args.source}:')
