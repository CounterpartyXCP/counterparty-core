...
def test_inverse_hash():
    ...
    assert (
        ...
        == "0dec68b0108886dee26e42caa5e4de13606c931f924abd4721f3e0a3396727b5"
    )
...
def test_script_to_asm():
    asm = utils.script_to_asm(
        b"v\xa9\x14H8\xd8\xb3X\x8cL{\xa7\xc1\xd0o\x86n\x9b79\xc607\x88\xac"
    )
    assert asm == [
        b"v",
        b"\xa9",
...
def test_decode_p2w():
    ...
    assert decode_p2w(script_pubkey)[0] == utils.script_to_address(
        script_pubkey, "mainnet"
    )

    script_pubkey = binascii.unhexlify(
        "0020dcbc2340bd1f6cc3ab0a3887020647ec471a279e3c889fb4414df30e3dd59f96"
    )
    assert decode_p2w(script_pubkey) == (
        "bc1qmj7zxs9arakv82c28zrsypj8a3r35fu7pure55",
        None,
    )
    assert decode_p2w(script_pubkey)[0] == utils.script_to_address(
        script_pubkey, "mainnet"
    ...
    assert decode_p2w(script_pubkey) == (
        "bc1qmlsh88wqwy0kfnkenx3rqe53l7v0lupc6q5xx6",
        None,
    )
    assert decode_p2w(script_pubkey)[0] == utils.script_to_address(
        script_pubkey, "mainnet"
