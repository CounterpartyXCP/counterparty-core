...
- **Correct the Bitcoin transaction fee calculation** (#3458). The Rust parser stopped walking a transaction's outputs at the first ordinary output after the Counterparty data (normally the change output), so any *further* outputs were dropped from the Bitcoin miner fee (`fee`) recorded for the transaction — e.g. `1db7a85e9bbbcd9f60a62411e94f1ae8d3851642d0e3ca73e095d522bf234293` was recorded as paying 19,388,665 sats while its inputs minus *all* outputs is 46,970 sats. Because `fee` participates in the `txlist_hash` consensus, the correction is gated behind the `correct_transaction_fee` protocol-change height: mainnet block **966,200** (~September 9, 2026), testnet3 block 5,166,000, testnet4 block 153,700 and signet block 321,300; regtest enables it immediately. Signet is gated like the other networks because its history ships in the bootstrap snapshots, and enabling the correction from block 0 there recomputes the fee of already-parsed transactions. Only the recorded fee changes: destinations, dispensed amounts and data are untouched.
