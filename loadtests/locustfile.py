...
class BlocksTasks(TaskSet):
    ...
    def get_last_block(self):
        ...
        with self.client.get("/v2/blocks/last?verbose=true&show_unconfirmed=false", headers=headers, catch_response=True) as response:
    ...
    def get_block_by_hash(self):
        ...
        if BlocksTasks.block_hash:
            self.client.get(f"/v2/blocks/{BlocksTasks.block_hash}?verbose=true&show_unconfirmed=false", headers=headers)
...
class TransactionsTasks(TaskSet):
    ...
    def get_transactions(self):
        ...
        with self.client.get("/v2/transactions?limit=10&cursor=200&verbose=true", headers=headers, catch_response=True) as response:
            if response.status_code == 200:
                result = response.json().get("result")
                if result:
    ...
    def get_transaction_info(self):
        ...
        if TransactionsTasks.tx_hash:
            self.client.get(f"/v2/transactions/{TransactionsTasks.tx_hash}?verbose=true", headers=headers)
...
class AddressesTasks(TaskSet):
    ...
    def get_balances_by_address(self):
        ...
        if TransactionsTasks.source_address:
            self.client.get(f"/v2/addresses/{TransactionsTasks.source_address}/balances?limit=10&cursor=200", headers=headers)

    @task
    def get_transactions_by_address(self):
        """Get transactions by stored source address."""
        if TransactionsTasks.source_address:
            self.client.get(f"/v2/addresses/{TransactionsTasks.source_address}/transactions?limit=10&cursor=200", headers=headers)
...
class UtxosTasks(TaskSet):
    ...
    def get_utxos_by_address(self):
        if TransactionsTasks.source_address:
            self.client.get(f"/v2/utxos/{TransactionsTasks.source_address}?limit=10&cursor=200", headers=headers)

# 6. Compose
class ComposeTasks(TaskSet):
    @task
    def compose_bet(self):
        self.client.post("/v2/compose/bet", json={"wager": 1000, "odds": 2, "counterparty": TransactionsTasks.source_address or "1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"}, headers=headers)

    @task
    def compose_broadcast(self):
        self.client.post("/v2/compose/broadcast", json={"value": 1.23, "source": TransactionsTasks.source_address or "1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa", "text": "Test Broadcast"}, headers=headers)
...
class OrdersTasks(TaskSet):
    ...
    def get_orders_by_address(self):
        ...
        if TransactionsTasks.source_address:
            self.client.get(f"/v2/orders?source={TransactionsTasks.source_address}", headers=headers)
...
class DispensesTasks(TaskSet):
    ...
    def get_dispenses_by_transaction_hash(self):
        ...
        if TransactionsTasks.tx_hash:
            self.client.get(f"/v2/transactions/{TransactionsTasks.tx_hash}/dispenses", headers=headers)

    @task
    def get_dispenses_by_source(self):
        """Get dispenses by source address."""
        if TransactionsTasks.source_address:
            self.client.get(f"/v2/dispenses/source/{TransactionsTasks.source_address}", headers=headers)
...
class BitcoinTasks(TaskSet):
    ...
    def get_unspent_txouts_by_address(self):
        if TransactionsTasks.source_address:
            self.client.get(f"/v2/bitcoin/{TransactionsTasks.source_address}/utxos", headers=headers)
