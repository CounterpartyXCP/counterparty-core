...
import server
from counterparty.lib import (config, api, util, exceptions, blocks, check, backend, database, transaction, script)
from counterparty.lib.messages import (send, order, btcpay, issuance, broadcast, bet, dividend, burn, cancel, rps, rpsresolve)
...
def run_scenario(scenario, rawtransactions_db):
    ...
    for tx in scenario:
        if tx[0] != 'create_next_block':
            module = sys.modules['counterparty.lib.messages.{}'.format(tx[0])]
...
def check_outputs(tx_name, method, inputs, outputs, error, records, counterpartyd_db):
    ...
    try:
        tested_module = sys.modules['counterparty.lib.{}'.format(tx_name)]
    except KeyError:    # TODO: hack
        tested_module = sys.modules['counterparty.lib.messages.{}'.format(tx_name)]
