...
CONFIG_ARGS = [
    ...
    [
        ("--backend-name",),
        {"default": "addrindex", "help": "the backend name to connect to"},
    ],
    [
        ("--backend-connect",),
    ...
    [
        ...
        {
            "type": int,
            "help": f"port on which to provide the {config.APP_NAME} JSON-RPC API",
        },
    ],
    [
        ("--rpc-user",),
    ...
    [
        ("--rpc-no-allow-cors",),
        {
            "action": "store_true",
            "default": False,
            "help": "allow ajax cross domain request",
        },
    ],
    [
        ("--rpc-batch-size",),
    ...
    [
        ("--database-file",),
        {"default": None, "help": "the path to the SQLite3 database file"},
    ],
    [
        ("--log-file",),
        {
...
def main():
    ...
    parser_server.add_argument(
        "--config-file", help="the path to the configuration file"
    )
    parser_server.add_argument(
        "--catch-up",
    ...
    parser_kickstart.add_argument(
        "--max-queue-size",
        type=int,
        help="Size of the multiprocessing.Queue for parsing blocks",
    )
    parser_kickstart.add_argument(
    ...
    parser_checkdb = subparsers.add_parser(
        "check-db", help="do an integrity check on the database"
    )
