...
def test_parse_order_invalid_data(ledger_db, blockchain_mock, defaults, test_helpers):
    ...
    print(
        helpers.to_json(
            ledger_db.execute(
                "SELECT * FROM orders WHERE tx_hash = ?", (tx["tx_hash"],)
            ).fetchone(),
            indent=4,
        )
    )
    print(
        helpers.to_json(
            ledger_db.execute(
                "SELECT * FROM order_matches WHERE id like ?", (f"%{tx['tx_hash']}%",)
            ).fetchone(),
            indent=4,
        )
    )
...
def test_parse_order_no_divisible(ledger_db, blockchain_mock, defaults, test_helpers):
    tx = blockchain_mock.dummy_tx(ledger_db, defaults["addresses"][1], fee=10000)
