...
CREATE VIEW  IF NOT EXISTS xcp_holders AS 
         ...
         UNION ALL 
            ...
            id AS escrow, CONCAT('bet_match_', CAST(rowid AS VARCAHR)) AS cursor_id,
         ...
         UNION ALL 
            ...
            tx_hash AS escrow, CONCAT('open_rps_', CAST(rowid AS VARCAHR)) AS cursor_id,
            'open_rps' AS holding_type, status
            FROM rps WHERE status = 'open'
         UNION ALL 
            SELECT 'XCP' AS asset, tx0_address AS address, wager AS quantity,
            id AS escrow, CONCAT('rps_match_', CAST(rowid AS VARCAHR)) AS cursor_id,
